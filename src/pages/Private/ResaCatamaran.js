...
import backGround from '../../assets/bg-catamaran.jpg'
...
export default function ResaCatamaran() {
    ...
    const addToLoisirs = async () => {
      ...
      if(validateName() && validatePhone() && validateKayak() && validateNumber()) {
        await setDoc(loisirsRef, {
          ...
          nom: name,
          phone: phone,
          catamaran1: stringKayak,
          nombre1: stringNumber,
          catamaran2: stringKayak2,
    ...
    <LoisirsNavBar />
      ...
      <Container maxWidth="50%" sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
        <Typography variant='h2' sx={{ textAlign: 'center', marginTop: '50px' }}>
          RÉSERVATION DE CATAMARAN (CARANTEC)
        </Typography>
        ...
        <div style={{ display: 'flex', gap: '25px', marginTop: '25px' }}>
          <FormControl sx={{ minWidth: 275 }}>
            <InputLabel id='select-cata-label'>Type de catamaran</InputLabel>
            <Select
              ...
              autoWidth
              label="Type de catamaran"
            >
              <MenuItem value='CatamaranSL52'>Catamaran de sport SL 5.2</MenuItem>
              <MenuItem value='CatamaranRS16'>Catamaran RS 16</MenuItem>
            </Select>
          </FormControl>
