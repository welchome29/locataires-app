...
import backGround from '../../assets/bg-planche.jpg'
...
export default function ResaPlanche() {
    ...
    const addToLoisirs = async () => {
      ...
      if(validateName() && validatePhone() && validateKayak() && validateNumber()) {
        await setDoc(loisirsRef, {
          activity: 'Planche à voile',
          nom: name,
          phone: phone,
          planche1: stringKayak,
          nombre1: stringNumber,
          planche2: stringKayak2,
    ...
    <>
      ...
      <Container maxWidth="50%" sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
        <Typography variant='h2' sx={{ textAlign: 'center', marginTop: '50px' }}>
          RÉSERVATION DE PLANCHE À VOILE (CARANTEC)
        </Typography>
        ...
        <div style={{ display: 'flex', gap: '25px', marginTop: '25px' }}>
          <FormControl sx={{ minWidth: 275 }}>
            <InputLabel id='select-cata-label'>Type de planche à voile</InputLabel>
            <Select
              labelId="select-cata-label"
              id="select-cata"
              open={open}
              onClose={handleClose}
              onOpen={handleOpen}
              ...
              autoWidth
              label="Type de planche à voile"
            >
              <MenuItem value='PlancheEcole'>Planche à voile école Bic Beach 225</MenuItem>
              <MenuItem value='PlanchePerf'>Planche à voile perf JP160, JP180 ou BIC 293 OD</MenuItem>
              <MenuItem value='PlancheFoil'>Planche à voile foil</MenuItem>
            </Select>
          </FormControl>
